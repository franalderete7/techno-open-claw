...
app.register(async (protectedApp) => {
  ...
  protectedApp.post("/v1/storefront/payment-intents", async (request, reply) => {
    const schema = z.object({
      ...
      delivery_mode: z.string().trim().max(80).optional().nullable(),
      availability_preference: z.string().trim().max(80).optional().nullable(),
      payment_preference: z.string().trim().max(80).optional().nullable(),
      customer_city: z.string().trim().max(120).optional().nullable(),
      customer_province: z.string().trim().max(120).optional().nullable(),
      contact_goal: z.string().trim().max(80).optional().nullable(),
      source_placement: z.string().trim().max(120).optional().nullable(),
    ...
    const intent = await createStorefrontPaymentIntent({
      ...
      buyerIntent: {
        ...
        availabilityPreference: body.availability_preference ?? null,
        paymentPreference: body.payment_preference ?? null,
        city: body.customer_city ?? null,
        province: body.customer_province ?? null,
        contactGoal: body.contact_goal ?? null,
        sourcePlacement: body.source_placement ?? null,
      },
      analytics: {
