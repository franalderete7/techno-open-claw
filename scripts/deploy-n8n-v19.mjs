...
const WORKFLOW_DIR = resolve(process.env.N8N_WORKFLOW_DIR || join(ROOT_DIR, "n8n/v19"));
const BACKUP_ROOT = resolve(
  process.env.N8N_BACKUP_DIR || join(ROOT_DIR, "n8n/backups/v19"),
);
const CONTAINER_TMP_DIR =
  process.env.N8N_CONTAINER_TMP_DIR || "/tmp/techno-open-claw-n8n-v19-deploy";
const N8N_API_BASE =
...
const CATALOG_WORKFLOW = {
  file: "TechnoStore_v19_catalog_only.json",
  name: "TechnoStore - AI Sales Agent v19 (catálogo)",
};

const EXPECTED_WORKFLOWS = [CATALOG_WORKFLOW];
...
function buildImportDir(tmpDir) {
  ...
  const sourcePath = join(WORKFLOW_DIR, CATALOG_WORKFLOW.file);
  const payload = JSON.parse(readFileSync(sourcePath, "utf8"));
  delete payload.id;
  payload.active = false;
  payload.isArchived = false;
  writeFileSync(join(importDir, CATALOG_WORKFLOW.file), `${JSON.stringify(payload, null, 2)}\n`);
...
async function main() {
  ...
  const tmpDir = mkdtempSync(join(tmpdir(), "techno-open-claw-n8n-v19-"));
  ...
  try {
    ...
    log("Backed up current v19 workflow (if any).");
    ...
    if (DRY_RUN) {
      ...
      log("Would import the single v19 catalog workflow.");
      log("Would unarchive if needed, then publish it.");
      log("");
    ...
    log("Importing v19 catalog workflow...");
    ...
    const catalogRecord = finalGrouped.get(CATALOG_WORKFLOW.name)?.canonical;
    if (!catalogRecord?.id) {
      throw new Error("Missing v19 workflow after import.");
    }
    if (catalogRecord.archived) {
      log(`Unarchiving workflow before publish: ${CATALOG_WORKFLOW.name} (${catalogRecord.id})`);
      await unarchiveWorkflow(n8nContainer, auth, catalogRecord.id);
    }
    log(`Publishing: ${CATALOG_WORKFLOW.name} (${catalogRecord.id})`);
    publishWorkflow(n8nContainer, auth, catalogRecord.id);
