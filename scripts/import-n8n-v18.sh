...
WORKFLOW_DIR="${N8N_WORKFLOW_DIR:-$ROOT_DIR/n8n/v18}"
CONTAINER_NAME="${N8N_CONTAINER_NAME:-}"
CONTAINER_TMP_DIR="${N8N_CONTAINER_TMP_DIR:-/tmp/techno-open-claw-n8n-v18}"
...
for workflow_file in "$WORKFLOW_DIR"/TechnoStore_v18_*.json; do
  if [[ ! -f "$workflow_file" ]]; then
    echo "No v18 workflow JSON files found in $WORKFLOW_DIR" >&2
    exit 1
  fi
...
echo
echo "Next:"
echo "1. Open n8n and verify the 7 v18 workflows exist."
echo "2. Relink child workflows in the entry workflow if needed."
echo "3. Confirm required env vars and credentials exist in n8n."
